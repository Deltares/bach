...
- The capacity of the link $e \in E_S$ is given by the smallest `max_flow_rate` of the nodes along the equivalent links in the subnetwork. If there are no nodes with a `max_flow_rate`, the link capacity is infinite;
...
- The sources are not all used at once. There is an optimization for each source priority used in the subnetwork. So if different sources have the same source priority, they are optimized for within the same optimization solve.
...
Ribasim.set_objective_demand_priority!(allocation_model, u, p, t, priority_idx)
